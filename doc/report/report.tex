...
\begin{secondpage}
    ...
    Built on \today~\currenttime~from \href{https://github.com/dmfrodrigues/feup-plog-tp2}{dmfrodrigues/feup-plog-tp2}, commit \input{COMMIT}\unskip.\par
...
Tests performed by the teacher during in-class demonstration in bench 5 confirmed the program's robustness.
...
Parameters $a$, $R_e$, $C$ and packet size were initially varied independently, Figure \ref{fig:S_a_Re}.
	...
	\begin{subfigure}[t]{0.38\textwidth}
		\begin{tikzpicture}[baseline]
			...
			\begin{axis}[
				...
				xmin=0, xmax=2.5, xtick distance=0.5, x tick label style={rotate=90},
				ylabel={Efficiency $S$},
				ymin=0, ymax=0.8,
				ytick distance=0.1,
			]
	...
	\begin{subfigure}[t]{0.31\textwidth}
		...
		\begin{tikzpicture}[baseline]
			...
			\begin{axis}[
				...
				grid=major,
				grid style={dashed,black!50},
				xmin=0, xmax=0.7, xtick distance=0.1, xlabel=Frame error rate $R_e$, x tick label style={rotate=90},
				ymin=0, ymax=0.8, ytick distance=0.1, yticklabels={,,}
			]
				\addplot[only marks, blue, mark=*, mark options={mark size=1.5pt}] table[x=Re,y=S,col sep=comma] {vary-fre.csv};
				\addplot[mark=none, black] {-0.64856632*x+0.63867854} node[right, align=left] at (0.13, 0.65) {$y=-0.6485663+0.6386785$\\$r^2=0.999914$};
			\end{axis}
		\end{tikzpicture}
	\end{subfigure}%
	\begin{subfigure}[t]{0.31\textwidth}
		...
		\begin{tikzpicture}[baseline]
			...
			\begin{axis}[
				...
				grid=major,
				grid style={dashed,black!50},
				xmin=0, xmax=25, xtick distance=5, xlabel=Inverse size $1/\overline{L_p}$ ($\SI{}{{\kibi\byte}^{-1}}$), x tick label style={rotate=90}, %scaled x ticks = false,
				ymin=0, ymax=0.8, ytick distance=0.1, yticklabels={,,}
			]
				\addplot[only marks, blue, mark=*, mark options={mark size=1.5pt}] table[x=1/size,y=S,col sep=comma] {vary-size.csv};
				\addplot[mark=none, black, domain=0:25] {-0.00971522*x+0.7723259} node[right, align=left] at (2.5, 0.45) {$y=-0.00971522+0.772326$\\$r^2=0.991206$};
			\end{axis}
		\end{tikzpicture}
	\end{subfigure}
	\caption{Efficiency $S$ plotted against $a$ (left), $R_e$ (center) and $N$ (right). On all graphs, $S$ is determined for each point from the average run time $T$ of four repetitions}\label{fig:S_a_Re}
\end{figure}
...
\begin{center}
	\begin{tikzpicture}[->,>=stealth',node distance=2.2cm,initial text=$ $,state/.style={circle, draw, minimum size=15mm}]
		\scriptsize
		\node[align=center, state, initial          					] (FLAG_RCV) 	{START};
		\node[align=center, state, below of=FLAG_RCV					] (A_RCV) 		{A\\RCV};
		...
		\draw	(FLAG_RCV)		edge[loop right				]	node{FLAG}					(FLAG_RCV)
				(FLAG_RCV)		edge[right					]	node{A\_SEND}				(A_RCV)
				(A_RCV)			edge[bend left, left		]	node{FLAG}					(FLAG_RCV)
