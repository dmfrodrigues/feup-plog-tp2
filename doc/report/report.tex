...
In this function we also have in consideration the placement of each student, with a penalty based on the grade if a student is not placed in his preferred option.

The final function:

\begin{gather} 
    c_i = \frac{1}{2}\left(1-\left(\frac{SumOdd}{SumEven}\right)^2\right) + \frac{1}{4}\left(1-\left(\frac{SumClass}{AvgSubject}\right)^2\right) \\
    s_i = \frac{1}{8}\sum_{n=i} ^ {StudentsInOption}{Grade_{n}} - \frac{1}{8}\sum_{n=i} ^ {StudentsNotInOption}{Grade_{n}} \\
    O = \sum{c_i} + \sum{s_i}
\end{gather}

\section{Solution presentation}

The solution visualization is made with the predicate:
...
We tested different options of search, changing the labelling parameters that controls the variables and solution selection in SICStus Prolog.
